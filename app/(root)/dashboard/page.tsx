...
import { useWalletAddress } from "@/hooks/useWalletAddresses";
...
export default function Dashboard() {
  ...
  const { address, loading } = useWalletAddress(selectedToken);
          ...
          return (
            ...
            >
              <div className="flex justify-start items-center mb-2 space-x-5">
                <span className="w-5 h-5 text-primary text-2xl">
              ...
              <span
                title={`${numericBalance} ${label}`}
                className="text-sm text-gray-300 py-3"
              >
        ...
        <HeadlessModal onClose={() => setOpenModal(null)}>
          {openModal === "deposit" ? (
            <div className="text-white space-y-4">
              <h2 className="text-xl mb-2 font-semibold text-black">
              ...
              {loading ? (
                <p className="text-black">Fetching address...</p>
              ) : address ? (
                <>
                  <div>
                    ...
                    <p className="bg-gray-800 text-white p-2 rounded break-all font-mono">
                      {address}
                    </p>
                    <button
                      onClick={() => navigator.clipboard.writeText(address)}
                      className="mt-2 px-3 py-1 bg-gray-600 hover:bg-gray-700 text-sm rounded"
                    >
                      Copy Address
                    </button>
                  </div>
                ...
                <p className="text-red-500">
                  Address not found for {selectedToken}
                </p>
              )}
            </div>
          ) : (
            ...
            >
              ...
              <button
                type="submit"
                className="w-full px-4 py-2 rounded bg-red-600 hover:bg-red-700 text-white transition"
              >
                Submit withdraw
              </button>
            </form>
